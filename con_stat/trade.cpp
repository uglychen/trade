...
void Trade::Init() {
    ...
    const Json::Value producer_order = config["producermq_order"];
    if (producer_order.empty()) {
        die("missing producer_order config");
    }

    const Json::Value producer_position = config["producermq_position"];
    if (producer_position.empty()) {
        die("missing producer_position config");
    }
    ...
    orderProducer_ = InitMQ(producer_order, false);
    positionProducer_ = InitMQ(producer_position, false);
    ...
    producer_order_exchange_ = producer_order.get("exchange", "").asString();
    producer_position_exchange_ = producer_position.get("exchange", "").asString();
...
void Trade::SendMessage(std::string msg, std::string type) {
    if (type == "order_book"){
        ...
        die_on_error(amqp_basic_publish(orderProducer_,
                                        1,
                                        amqp_cstring_bytes(producer_order_exchange_.c_str()),
                                        amqp_cstring_bytes(""),
                                        0,
                                        0,
                                        NULL,
        ...
        die_on_error(amqp_basic_publish(positionProducer_,
                                        1,
                                        amqp_cstring_bytes(producer_position_exchange_.c_str()),
                                        amqp_cstring_bytes(""),
                                        0,
                                        0,
                                        NULL,
...
bool Trade::HeartBeat( )
{
    ...
    if(now > (next_heart_time + (10*1000000)))
    {
        ...
        res = amqp_send_frame(orderBookProducer_, &heartbeat);
        if (AMQP_STATUS_OK != res) {
            LOG(ERROR) << "orderBookProducer_ heart_failed: " << now;
            return false;
        }
        
        res = amqp_send_frame(orderProducer_, &heartbeat);
        if (AMQP_STATUS_OK != res) {
            LOG(ERROR) << "orderProducer_ heart_failed: " << now;
            return false;
        }
        
        res = amqp_send_frame(positionProducer_, &heartbeat);
        if (AMQP_STATUS_OK != res) {
            LOG(ERROR) << "positionProducer_ heart_failed: " << now;
            return false;
        }

        res = amqp_send_frame(tradeProducer_, &heartbeat);
        if (AMQP_STATUS_OK != res) {
            LOG(ERROR) << "tradeProducer_ heart_failed: " << now;
            return false;
        }

        res = amqp_send_frame(klineProducer_, &heartbeat);
        if (AMQP_STATUS_OK != res) {
            LOG(ERROR) << "klineProducer_ heart_failed: " << now;
            return false;
        }

        res = amqp_send_frame(tickerProducer_, &heartbeat);
        if (AMQP_STATUS_OK != res) {
            LOG(ERROR) << "tickerProducer_ heart_failed: " << now;
            return false;
        }
...
void Trade::CleanUp() {
    ...
    die_on_amqp_error(amqp_channel_close(orderProducer_, 1, AMQP_REPLY_SUCCESS), "Closing channel");
    die_on_amqp_error(amqp_connection_close(orderProducer_, AMQP_REPLY_SUCCESS), "Closing connection");
    die_on_error(amqp_destroy_connection(orderProducer_), "Ending connection");

    die_on_amqp_error(amqp_channel_close(positionProducer_, 1, AMQP_REPLY_SUCCESS), "Closing channel");
    die_on_amqp_error(amqp_connection_close(positionProducer_, AMQP_REPLY_SUCCESS), "Closing connection");
    die_on_error(amqp_destroy_connection(positionProducer_), "Ending connection");
